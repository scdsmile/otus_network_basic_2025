...
>файла vlan.dat нет. Вероятно это потому, что данные о vlan хранятся в общем конфигурационном файле, а не в отдельном.
...
> здесь бы вводилась команда **delete vlan.dat**, но смысла в этом нет 😀
...
####	Перезагрузите коммутатор, чтобы удалить устаревшую информацию о конфигурации из памяти. Затем появится запрос на подтверждение перезагрузки коммутатора. Нажмите клавишу Enter, чтобы продолжить.
После перезагрузки коммутатора появится запрос о входе в диалоговое окно начальной конфигурации. Чтобы ответить, введите no и нажмите клавишу Enter.
...
```
>Стартап конфиг коммутатора очищен, после перезагрузки коммутатор сброшен в первоначальное состояние.
